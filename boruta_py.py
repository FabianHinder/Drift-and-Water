...
y = np.linspace(0,2*np.pi,X.shape[0])
y = np.array([(np.sin(n*y),np.cos(n*y)) for n in range(1,5)])
y = y.reshape(y.shape[0]*y.shape[1],y.shape[2]).T

from sklearn.ensemble import RandomForestRegressor, RandomForestClassifier
print(BorutaPy(RandomForestRegressor(max_depth=10), verbose=True).fit(X,y).stat)
'''